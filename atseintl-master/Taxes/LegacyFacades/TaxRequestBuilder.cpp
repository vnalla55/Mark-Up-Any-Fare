...
template <typename T>
const T& as_const(T& r) { return r; }

struct SegFareInfo
{
  SegFareInfo(int16_t _index,
              tse::TravelSeg* _tseTravelSeg,
              tse::FareUsage* _tseFareUsage,
              InputMapping* _mapping)
    : index(_index), tseTravelSeg(_tseTravelSeg), tseFareUsage(_tseFareUsage), mapping(_mapping)
  {
  }
  bool operator<(const SegFareInfo& rhs) const { return index < rhs.index; }
...
Logger
_logger("atseintl.AtpcoTaxes.TaxRequestBuilder");

tax::type::Index
addGeoPathMapping(tax::InputGeoPathMapping* geoPathMapping,
                  tax::InputRequest::InputGeoPathMappings& geoPathMappings)
{
  tax::type::Index foundId = 0;
  if (FacadesUtils::findGeoPathMapping(*geoPathMapping, geoPathMappings, foundId))
  {
...
void
processInputTseFareUsage(tse::PricingTrx& trx,
                         ...
                         const tse::FareUsage& tseFareUsage,
                         InputFarePath& farePath,
                         boost::ptr_vector<InputFare>& fares)
{
  ...
  {
    FareClassCode code = tseFareUsage.paxTypeFare()->fare()->fareInfo()->fareClass();
    ...
    if (LIKELY(fare->_basis.find('/') == std::string::npos))
    {
      ...
      {
        const int16_t segOrd = tseItin.segmentOrder(tseFareUsage.travelSeg().front());
        tktDesignator = trx.getRequest()->tktDesignator(segOrd).c_str();
        if (LIKELY(tktDesignator.empty()))
          tktDesignator = trx.getRequest()->specifiedTktDesignator(segOrd).c_str();
      }
    ...
    fare->_directionality = toTaxDirectionality(tseFareUsage.paxTypeFare()->directionality());
    fare->_oneWayRoundTrip = tseFareUsage.paxTypeFare()->owrt();
    fare->_sellAmount = 0;
    fare->_isNetRemitAvailable = false;
    const FareClassAppInfo* fca = tseFareUsage.paxTypeFare()->fareClassAppInfo();
    if (LIKELY(fca))
    {
        ...
        tseFareUsage.paxTypeFare()->isDiscounted())
    {
      ...
      catch (...) {} // discountInfo throws if no rule data, that's normal
    }
    if (!isCat19Discount)
      fare->_outputPtc = toTaxPassengerCode(tseFareUsage.paxTypeFare()->actualPaxType()->paxType());

    bool noConversion = tseFareUsage.paxTypeFare()->currency() == paymentCurrency &&
                        baseFareCurrency.empty() &&
                        calculationCurrency.empty();
    const tse::CurrencyServiceV2 currencyService(trx, trx.getRequest()->ticketingDT());
    try
    {
      if (noConversion)
    ...
    {
      const NegPaxTypeFareRuleData* negPaxTypeFare = tseFareUsage.paxTypeFare()->getNegRuleData();
      if (negPaxTypeFare && negPaxTypeFare->nucNetAmount() > 0)
      {
        try
        {
          if (noConversion)
            fare->_sellAmount = tax::doubleToAmount(negPaxTypeFare->netAmount());
          else
            fare->_sellAmount = currencyService.convert(negPaxTypeFare->netAmount(),
                                                        baseFareCurrency,
                                                        calculationCurrency,
                                                        paymentCurrency,
                                                        CurrencyConversionRequest::FARES,
...
void
processInputTseTravelSegForFlight(const tse::TravelSeg& tseTravelSeg,
                                  const tse::TravelSeg* const previousTseTravelSeg,
                                  const tse::Itin& tseItin,
                                  const tse::FareUsage& tseFareUsage,
                                  ...
                                  bool afterHiddenStop = false)
{
  ...
  if (unticketedTag == type::UnticketedTransfer::No)
  {
    flight->_departureTime = boost::posix_time::time_duration(
        tseTravelSeg.departureDT().hours(), tseTravelSeg.departureDT().minutes(), 0, 0);
    flight->_arrivalTime = boost::posix_time::time_duration(
        tseTravelSeg.arrivalDT().hours(), tseTravelSeg.arrivalDT().minutes(), 0, 0);
    flight->_arrivalDateShift = Convert::longToShort(
  ...
  {
    flight->_operatingCarrier = toTaxCarrierCode(airSeg->operatingCarrierCode());
    flight->_marketingCarrier = toTaxCarrierCode(airSeg->marketingCarrierCode());
    flight->_marketingCarrierFlightNumber = Convert::intToShort(airSeg->marketingFlightNumber());
  }
  ...
  if (!afterHiddenStop)
  {
    flightUsage->_connectionDateShift = Convert::longToShort(
        (nullptr != previousTseTravelSeg)
            ? (tseTravelSeg.departureDT().date() - previousTseTravelSeg->arrivalDT().date()).days()
            : 0);
  }
...
void
createGeo(const tse::TravelSeg& tseTravelSeg,
          const tse::Loc& loc,
          type::TaxPointTag taxPointTag,
          type::UnticketedTransfer unticketedTag,
          TaxRequestBuilder_DEPRECATED::TravelSegGeoItems& items,
          boost::ptr_vector<InputGeo>& geos,
          boost::ptr_vector<InputMap>& maps)
{
...
void
processInputTseTravelSeg(const tse::Itin& tseItin,
                         const tse::FareUsage& tseFareUsage,
                         tse::TravelSeg* travelSeg,
                         const tse::TravelSeg* previousTravelSeg,
                         boost::ptr_vector<InputGeo>& geos,
                         TaxRequestBuilder_DEPRECATED::TravelSegGeoItems& items,
                         boost::ptr_vector<InputFlight>& flights,
                         boost::ptr_vector<InputFlightUsage>& flightUsages,
                         InputMapping& mapping)
{
  ...
  for (const tse::Loc* hiddenStop: travelSeg->hiddenStops())
  {
    ...
    processInputTseTravelSegForFlight(*travelSeg,
                                      previousTravelSeg,
                                      tseItin,
                                      tseFareUsage,
                                      flights,
                                      flightUsages,
                                      type::UnticketedTransfer::Yes,
                                      afterHiddenStop);
...
void
processInputTseItin(tse::PricingTrx& trx,
                    const tse::Itin& tseItin,
                    const tse::FarePath& tseFarePath,
                    InputRequest& taxRequest,
                    InputItin& itin,
                    InputFarePath& farePath,
                    InputGeoPath& geoPath,
                    InputFlightPath& flightPath,
                    TaxRequestBuilder_DEPRECATED::TravelSegGeoItems& items,
                    const type::CarrierCode& validatingCarrier)
{
    ...
    for (tse::FareUsage* tseFareUsage: tsePricingUnit->fareUsage())
    {
      ...
      for (tse::TravelSeg* tseTravelSeg: tseFareUsage->travelSeg())
      {
        ...
        segFareInfos.push_back(
            SegFareInfo(tseItin.segmentOrder(tseTravelSeg), tseTravelSeg, tseFareUsage, &mapping));
      }
      ...
      if (LIKELY(nullptr != ptf))
      {
        ...
        try
        {
          if (ptf->isFareByRule() && !ptf->isSpecifiedFare() && (baseFare = ptf->baseFare(25)) != nullptr)
          {
            totalAmountBeforeDiscount += std::max(baseFare->nucFareAmount(), ptf->nucFareAmount());
          }
          else
          {
      ...
      {
        processInputTseFareUsage(trx,
                                 (tseFarePath.baseFareCurrency()),
  ...
  {
    processInputTseTravelSeg(tseItin,
                             *segFareInfo.tseFareUsage,
                             segFareInfo.tseTravelSeg,
                             prevTravelSeg,
                             geoPath._geos,
                             items,
                             taxRequest.flights(),
                             flightPath._flightUsages,
                             *segFareInfo.mapping);
  ...
  try
  {
    if (noConversion)
    {
      farePath._totalAmount = tax::doubleToAmount(tseFarePath.getTotalNUCAmount());
      if (!fallback::markupAnyFareOptimization(&trx))
      {
        farePath._totalMarkupAmount = tax::doubleToAmount(tseFarePath.getTotalNUCMarkupAmount());
      }
      farePath._totalAmountBeforeDiscount = tax::doubleToAmount(totalAmountBeforeDiscount);
    }
    else
    {
      farePath._totalAmount =
        currencyService.convert(tseFarePath.getTotalNUCAmount(),
                                tseFarePath.baseFareCurrency(),
                                tseFarePath.calculationCurrency(),
                                toTseCurrencyCode(taxRequest.ticketingOptions().paymentCurrency()),
                                CurrencyConversionRequest::FARES,
      ...
      if (!fallback::markupAnyFareOptimization(&trx))
      {
        farePath._totalMarkupAmount =
                currencyService.convert(tseFarePath.getTotalNUCMarkupAmount(),
                                        tseFarePath.baseFareCurrency(),
                                        tseFarePath.calculationCurrency(),
                                        toTseCurrencyCode(taxRequest.ticketingOptions().paymentCurrency()),
                                        CurrencyConversionRequest::FARES,
      ...
      farePath._totalAmountBeforeDiscount =
        currencyService.convert(totalAmountBeforeDiscount,
                                tseFarePath.baseFareCurrency(),
                                tseFarePath.calculationCurrency(),
                                toTseCurrencyCode(taxRequest.ticketingOptions().paymentCurrency()),
                                CurrencyConversionRequest::FARES,
  ...
  const type::Index geoPathMappingId =
      addGeoPathMapping(geoPathMapping, taxRequest.geoPathMappings());
  ...
  const bool pccOverride =
    tseItin.agentPCCOverride() != nullptr &&
    !equal(taxRequest.pointsOfSale()[0].agentCity(), tseItin.agentPCCOverride()->agentCity());

  if (pccOverride)
  {
    std::unique_ptr<InputPointOfSale> pos = InputPointOfSaleFactory::createInputPointOfSale(
        trx, *tseItin.agentPCCOverride(), taxRequest.pointsOfSale().size());
  ...
  itin._travelOriginDate = boost::gregorian::date(origin.year(), origin.month(), origin.day());
...
void
addChangeFee(const type::MoneyAmount& amount, InputRequest& inputRequest, InputItin& itin)
{
...
void
proccessChangeFee(const tse::FarePath& tseFarePath,
                  tse::PricingTrx& trx,
                  ...
                  InputItin& itin)
{
  ...
  if ((tseFarePath.reissueCharges() != nullptr) && (trx.excTrxType() == PricingTrx::AR_EXC_TRX) &&
      (tseFarePath.reissueCharges()->changeFee > tse::EPSILON) &&
      !tseFarePath.ignoreReissueCharges())
  {
    type::MoneyAmount changeFeeAmount = doubleToAmount(tseFarePath.reissueCharges()->changeFee);
    const tse::CurrencyCode& changeFeeCurrency = tseFarePath.reissueCharges()->changeFeeCurrency;

    if (inputRequest.ticketingOptions().paymentCurrency() != toTaxCurrencyCode(changeFeeCurrency))
    {
      ...
      const tse::CurrencyServiceV2 currencyService(trx, trx.getRequest()->ticketingDT());
      changeFeeAmount =
          currencyService.convertTo(inputRequest.ticketingOptions().paymentCurrency(), source);
    }
...
void
addPassenger(const tse::PricingTrx& trx,
             const tse::DateTime& ticketingDT,
             const tse::PricingOptions* options,
             const tse::PaxType& paxType,
             boost::ptr_vector<InputPassenger>& passengers)
{
  ...
  if (LIKELY(options))
  {
    passenger->_passengerStatus._nationality = tse::toTaxNationCode(options->nationality());
    passenger->_passengerStatus._employment = tse::toTaxNationCode(options->employment());
    passenger->_passengerStatus._residency = tse::toTaxLocZoneCode(options->residency());
  }
  ...
  if (paxType.age() != 0)
  {
    passenger->_birthDate = boost::gregorian::date(
        ticketingDT.year(), ticketingDT.month(), ticketingDT.day());

    passenger->_birthDate -= boost::gregorian::years(paxType.age());
  }
  else // paxType.age() == 0, age unknown (except infants, where this is ok)
  {
    static const std::vector<std::string> infantPtc =
      {"CBI", "INE", "FBI", "FGI", "FNF", "INF", "INR",
       "INS", "INY", "ISR", "ITF", "ITS", "JNF", "JNS",
       "LIF", "LNS", "MNF", "MNS", "ZEI", "GIF", "GIS",
       "INX", "MIF", "MSS", "TNF", "WBI", "WBS"};
    static const std::vector<std::string> childPtc =
      {"CBC", "CHR", "CNN", "CSB", "DNN", "ECH", "ENN",
       "FNN", "GNN", "INN", "JNN", "LNN", "MNN", "PNN",
       "TNN", "UNN", "VNN", "YNN", "ZEC", "CCA", "FBC",
       "FGC", "MDP", "MIC", "UNR", "WBC", "CNE"};

    if (!fallback::fallbackAtpcoTaxDefaultAgeForADTCNN(&trx)) // inf = 0, cnn = 2, other = 18
    {
      passenger->_birthDate = boost::gregorian::date(
                  Convert::intToShort(ticketingDT.year()), ticketingDT.month(), ticketingDT.day());

      if (std::find(childPtc.begin(), childPtc.end(), paxType.paxType()) != childPtc.end())
        passenger->_birthDate -= boost::gregorian::years(2);
      else if (std::find(infantPtc.begin(), infantPtc.end(), paxType.paxType()) == infantPtc.end())
        passenger->_birthDate -= boost::gregorian::years(18);
    }
    else // default age for inf = 0, other blank date, strict matching
    {
      if (std::find(infantPtc.begin(), infantPtc.end(), paxType.paxType()) != infantPtc.end())
        passenger->_birthDate =
              boost::gregorian::date(Convert::intToShort(ticketingDT.year()),
                                     ticketingDT.month(),
                                     ticketingDT.day());
      else
        passenger->_birthDate = boost::gregorian::date(boost::gregorian::not_a_date_time);
    }
  }
...
void
processInputDiagnostic(tse::Diagnostic& tseDiagnostic, tax::InputDiagnosticCommand& diagnostic)
{
  ...
  for (DiagParamMap_iterator it = parameters.begin(); it != parameters.end(); it++)
  {
...
getRequestedCurrency(tse::PricingTrx& trx)
{
...
void
processInputCommonHeader(tse::PricingTrx& trx, InputRequest& request)
{
  ...
  std::unique_ptr<InputPointOfSale> pointOfSale = InputPointOfSaleFactory::createInputPointOfSale(
      trx, *ticketingAgent, request.pointsOfSale().size());
  ...
  request.ticketingOptions().ticketingPoint() = request.pointsOfSale().back().loc();
  if (!trx.getRequest()->ticketPointOverride().empty())
  {
    request.ticketingOptions().ticketingPoint() = toTaxAirportCode(trx.getRequest()->ticketPointOverride());
  }
  ...
  request.ticketingOptions().ticketingDate() = boost::gregorian::date(
      trx.ticketingDate().year(), trx.ticketingDate().month(), trx.ticketingDate().day());
  ...
  {
    request.processing()._tch =
      trx.getRequest()->getSettlementMethod() == "TCH" ||
      (trx.countrySettlementPlanInfo() &&
       trx.countrySettlementPlanInfo()->getSettlementPlanTypeCode() == "TCH");
  ...
  if (trx.getRequest()->isExemptSpecificTaxes() && trx.getRequest()->taxIdExempted().empty())
  {
    InputCalculationRestriction* cr = new InputCalculationRestriction();
  ...
  if (trx.getRequest()->isExemptSpecificTaxes() && !trx.getRequest()->taxIdExempted().empty())
  {
    std::unique_ptr<InputCalculationRestriction> cr(new InputCalculationRestriction());
...
tse::FarePath&
selectGsaFarePath(tse::FarePath& mainFarePath, const tse::CarrierCode& validatingCarrier)
{
  tse::FarePath* alternative = mainFarePath.findTaggedFarePath(validatingCarrier);
...
void
logInputValCarriers(std::ostream* gsaDiag, tse::Itin& tseItin, tse::FarePath& tseFarePath)
{
  if (gsaDiag)
  {
    *gsaDiag << separator << "STAGE: BUILD REQUEST\n";
    *gsaDiag << "  ITIN: " << address(&tseItin) << "  FPATH: " << address(&tseFarePath) << "\n";
...
class TaxRequestBuilder_DEPRECATED::Impl // implementation details for TaxRequestBuilder
{
public:
  static void processFarePathForItin(tse::PricingTrx& trx,
                                     const ServicesFeesMap& servicesFees,
                                     tse::Itin&,
                                     tse::FarePath&,
                                     const std::vector<type::Index>& arunksBeforeSeg,
                                     const std::vector<type::Index>& hiddenBeforeSeg,
                                     V2TrxMappingDetails& v2Mapping,
                                     InputRequest& request,
                                     const type::CarrierCode& validatingCarrier,
                                     tse::FarePath* mainPath /* can be null */);

  static void process(tse::PricingTrx& trx,
                      const ServicesFeesMap& servicesFees,
                      V2TrxMappingDetails& v2Mapping,
                      InputRequest& inputRequest,
                      std::ostream* gsaDiag = nullptr);

  static void processMainAndNetFarePath(tse::PricingTrx& trx,
                                        const ServicesFeesMap& servicesFees,
                                        tse::Itin&,
                                        tse::FarePath&,
                                        const std::vector<type::Index>& arunksBeforeSeg,
  ...
  static void processTopLevelFarePath(tse::PricingTrx& trx,
                                      const ServicesFeesMap& servicesFees,
                                      tse::Itin&,
                                      tse::FarePath&,
                                      const std::vector<type::Index>& arunksBeforeSeg,
...
void
TaxRequestBuilder_DEPRECATED::Impl::process(tse::PricingTrx& trx,
                                 const ServicesFeesMap& servicesFees,
                                 V2TrxMappingDetails& v2Mapping,
                                 InputRequest& inputRequest,
                                 std::ostream* gsaDiag)
{
  if (gsaDiag) *gsaDiag << separator << std::string(20, ' ') << "GSA DIAGNOSTIC\n";

  processApplicableGroups(trx, trx.atpcoTaxesActivationStatus(), inputRequest.processing());
  processInputCommonHeader(trx, inputRequest);
  processInputDiagnostic(trx.diagnostic(), inputRequest.diagnostic());

  ItinSelector itinSelector(trx);

  for (tse::Itin* tseItin: itinSelector.get())
  {
    std::vector<type::Index> arunksBeforeSeg = computeArunksBeforeSeg(tseItin->travelSeg());
    std::vector<type::Index> hiddenBeforeSeg = computeHiddenBeforeSeg(tseItin->travelSeg());
    for (tse::FarePath* tseFarePath: tseItin->farePath())
    {
      processTopLevelFarePath(
          trx, servicesFees, *tseItin, *tseFarePath, arunksBeforeSeg, hiddenBeforeSeg, v2Mapping, inputRequest, gsaDiag);
    }
  }
}

void
TaxRequestBuilder_DEPRECATED::Impl::processTopLevelFarePath(tse::PricingTrx& trx,
                                                 const ServicesFeesMap& servicesFees,
                                                 tse::Itin& tseItin,
                                                 ...
                                                 std::ostream* gsaDiag)
{
  const bool gsaLogicEnabled = trx.isValidatingCxrGsaApplicable() &&
                               trx.atpcoTaxesActivationStatus().isTaxOnItinYqYrTaxOnTax();

  const std::vector<tse::CarrierCode>& valCxrs = as_const(tseFarePath).validatingCarriers();
  ...
  if (doGsa)
  {
    ...
    {
      tse::FarePath& workingFarePath = selectGsaFarePath(tseFarePath, alternateValCxr);
      processMainAndNetFarePath(trx,
                                servicesFees,
                                tseItin,
                                workingFarePath,
                                arunksBeforeSeg,
                                ...
                                v2Mapping,
                                inputRequest,
                                toTaxCarrierCode(alternateValCxr),
                                &tseFarePath);
    }
  }
  else
  {
    processMainAndNetFarePath(trx,
                              ...
                              v2Mapping,
                              inputRequest,
                              toTaxCarrierCode(tseItin.validatingCarrier()),
                              nullptr);
  }
}

void
TaxRequestBuilder_DEPRECATED::Impl::processMainAndNetFarePath(tse::PricingTrx& trx,
                                                   const ServicesFeesMap& servicesFees,
                                                   tse::Itin& tseItin,
                                                   tse::FarePath& tseFarePath,
                                                   const std::vector<type::Index>& arunksBeforeSeg,
                                                   ...
                                                   tse::FarePath* mainPath /* can be null */)
{
  ...
  if (TrxUtil::isCat35TFSFEnabled(trx) && tseFarePath.netFarePath())
  {
    processFarePathForItin(trx,
                           servicesFees,
                           tseItin,
                           *tseFarePath.netFarePath(),
                           arunksBeforeSeg,
                           hiddenBeforeSeg,
                           v2Mapping,
                           inputRequest,
  ...
  if (fallback::markupAnyFareOptimization(&trx))
  {
    if (tseFarePath.adjustedSellingFarePath())
    {
      processFarePathForItin(trx,
                             servicesFees,
                             tseItin,
                             *tseFarePath.adjustedSellingFarePath(),
                             arunksBeforeSeg,
                             hiddenBeforeSeg,
                             v2Mapping,
                             inputRequest,
...
void
TaxRequestBuilder_DEPRECATED::Impl::processFarePathForItin(tse::PricingTrx& trx,
                                                const ServicesFeesMap& servicesFees,
                                                tse::Itin& tseItin,
                                                tse::FarePath& tseFarePath,
                                                const std::vector<type::Index>& arunksBeforeSeg,
                                                ...
                                                tse::FarePath* mainPath /* can be null */)
{
  ...
  proccessChangeFee(tseFarePath, trx, inputRequest, *itin);

  TaxRequestBuilder_DEPRECATED::InputGeoMap inputGeoMap;

  const tax::ItinFarePathKey key = std::make_tuple(&tseItin, &tseFarePath, validatingCarrier, itin->_id);
  v2Mapping._itinFarePathMapping.push_back(key);

  processInputTseItin(trx,
                      tseItin,
                      tseFarePath,
                      inputRequest,
                      *itin,
  ...
  addPassenger(trx,
               ...
               trx.getOptions(),
               *tseFarePath.paxType(),
               inputRequest.passengers());

  tax::ServicesFeesMapKey serviceFeesKey = std::make_tuple(&tseItin, &tseFarePath, validatingCarrier);
  tax::ServicesFeesMap::const_iterator it = servicesFees.find(serviceFeesKey);
  if (it != servicesFees.end())
  {
    addYqYrServicesFees(it->second, *itin, inputRequest, arunksBeforeSeg, hiddenBeforeSeg, tseItin.travelSeg());
  }

  if (taxUtil::isAnciliaryItin(trx, tseItin))
  {
    OptionalServicesForOtaTaxReqBuilder optionalServicesForOtaTaxReqBuilder(
        trx, inputRequest, *itin, tseFarePath);
    optionalServicesForOtaTaxReqBuilder.addOptionalServices();
  }
  else
  {
    OptionalServicesBuilder optionalServicesBuilder(trx,
                                                    inputRequest,
                                                    *itin,
                                                    inputGeoMap[key],
                                                    tseFarePath,
                                                    v2Mapping._optionalServicesRefs,
                                                    v2Mapping._ocTaxInclIndMap);

    const AtpcoTaxesActivationStatus& status = trx.atpcoTaxesActivationStatus();
...
TaxRequestBuilder_DEPRECATED::computeArunksBeforeSeg(const std::vector<TravelSeg*>& segments)
{
...
std::vector<type::Index>
TaxRequestBuilder_DEPRECATED::computeHiddenBeforeSeg(const std::vector<TravelSeg*>& segments)
{
  std::vector<type::Index> ans;
  ans.reserve(segments.size());
  type::Index count = 0;
...
InputRequest*
TaxRequestBuilder_DEPRECATED::buildInputRequest(tse::PricingTrx& trx,
                                     const ServicesFeesMap& servicesFees,
                                     V2TrxMappingDetails& v2MappingDetails,
                                     std::ostream* gsaDiag) const
{
  InputRequest* request = nullptr;
  trx.dataHandle().get(request);

  Impl::process(trx, servicesFees, v2MappingDetails, *request, gsaDiag);

  return request;
}

InputRequest*
TaxRequestBuilder_DEPRECATED::buildInputRequest(tse::ShoppingTrx& trx,
                                     const ServicesFeesMap& servicesFees,
                                     V2TrxMappingDetails& v2MappingDetails) const
{
  ...
  Impl::process(trx, servicesFees, v2MappingDetails, *request);

  return request;
}

bool
FareDisplayUtil::checkTicketingAgentInformation(tse::PricingTrx& trx)
{
  return trx.getRequest() && trx.getRequest()->ticketingAgent() &&
         trx.getRequest()->ticketingAgent()->agentLocation() &&
         !trx.getRequest()->ticketingAgent()->agentLocation()->nation().empty();
}

void
TaxRequestBuilder_DEPRECATED::addYqYrServicesFees(const tax::ServicesFeesMapVal& items,
                                       tax::InputItin& itin,
                                       InputRequest& inputRequest,
                                       const std::vector<type::Index>& arunksBeforeSeg,
                                       const std::vector<type::Index>& hiddenBeforeSeg,
                                       const std::vector<TravelSeg*>& travelSegs)
{
  ...
  {
    processYqYrTaxItem(*taxItem, inputRequest, *geoPathMapping, *yqYrPath, arunksBeforeSeg, hiddenBeforeSeg, travelSegs);
  }

  itin._yqYrPathGeoPathMappingRefId =
      addGeoPathMapping(geoPathMapping, inputRequest.geoPathMappings());
}

void
TaxRequestBuilder_DEPRECATED::processYqYrTaxItem(const tse::TaxItem& taxItem,
                                      InputRequest& inputRequest,
                                      ...
                                      const std::vector<TravelSeg*>& travelSegs)
{
  if (FacadesUtils::isYqYr(taxItem.taxCode()))
  {
    ...
    buildYqYrMappings(taxItem, geoPathMapping, arunksBeforeSeg, hiddenBeforeSeg, travelSegs);
    // TOTO analyzer geoPathMapping._mappings.size() == yqYrPath->_yqYrUsages.size()
  }
}

tax::type::Index
TaxRequestBuilder_DEPRECATED::addYqYr(const tse::TaxItem& taxItem, InputRequest::InputYqYrs& yqYrs)
{
  tax::InputYqYr yqYr;
...
void
TaxRequestBuilder_DEPRECATED::processApplicableGroups(PricingTrx& trx,
                                           ...
                                           InputProcessingOptions& options)
{
  ...
  if (status.isTaxOnItinYqYrTaxOnTax())
  {
    options.addApplicableGroup(makeGroup(3));
  }

  if (status.isTaxOnChangeFee())
  {
...
void
TaxRequestBuilder_DEPRECATED::buildYqYrMappings(const tse::TaxItem& taxItem,
                                     tax::InputGeoPathMapping& geoPathMapping,
                                     const std::vector<type::Index>& arunksBeforeSeg,
                                     const std::vector<type::Index>& hiddenBeforeSeg,
                                     const std::vector<TravelSeg*>& travelSegs)
{
  ...
  for (tax::type::Index id = taxItem.travelSegStartIndex(); id <= taxItem.travelSegEndIndex(); ++id)
  {
    const TravelSeg* travelSeg = travelSegs[id];
    const tse::AirSeg* airSeg = travelSeg->toAirSeg();
    if (LIKELY(nullptr != airSeg))
    {
      tax::type::Index geoId = (id - arunksBeforeSeg[id] + hiddenBeforeSeg[id]) * 2;
