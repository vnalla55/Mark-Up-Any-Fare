"""
Do not write over 100 columns per a line.
...
def RunRule(lexer, line, lineno) :
    if not Match("^\s*$", line) :
        if len(line) > 100 :
            nsiqcppstyle_reporter.Error(DummyToken(lexer.filename, line, lineno, 0), __name__, 'Lines should very rarely be longer than 100 characters')
