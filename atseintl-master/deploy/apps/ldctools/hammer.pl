...
while($_ = shift @args) {
	if($_ =~ /^--piili0\d\d$/) {
		...
		$map_req_to_response = shift @args or die "must specify a filename after $_ option";
	} elsif($_ eq '--silent') {
		$print_responses = 0;
...
OPTIONS:
	...
	-a, --async: send all requests at the same time
        ...
        --retry: keep retrying same transaction on connect errors 
...
if($warm_first) {
	...
	foreach my $request (@requests) {
                my $actual_request="" ;
		my $sock = &send_data($request,$host,$port,\$actual_request);
...
foreach my $request (@requests) {
	...
	if($pid == 0) {
		...
		} elsif($map_req_to_response ne "") {
                        $actual_request =~ s/\n//;
                        open FILE, ">>$map_req_to_response" or die "Count not open $map_req_to_response: $!";
			print FILE "$actual_request {{{--------}}} $response\n";
                        close FILE;
...
print STDERt
"all requests processed in $taken seconds\n";
...
sub send_data
{
	my ($request,$host,$port,$actual_request_ref) = @_;
	...
	if($print_request) {
	  ...
	  $sock = new IO::Socket::INET(PeerAddr => $host, PeerPort => $port, Proto => 'tcp' ) ;
          if( not $sock )
          {
            print STDERR "Could not connect to $host:$port\n" ;
            if($retry_connect)
            {
              print STDERR "Waiting to retry connection...\n" ;
              sleep 1 ;
            }
            else
            {
              print STDERR "Request aborted.\n" ;
              exit ;
            }
          }
        }
