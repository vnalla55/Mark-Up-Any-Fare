...
def LLVMSymbolizerFactory(system, addr):
  ...
  if not symbolizer_path:
    ...
    if not symbolizer_path:
      ...
      symbolizer_path = os.path.join(script_dir, 'llvm-symbolizer')
      if not os.path.isfile(symbolizer_path):
...
def filter(name):
    format = '(/opt/atseintl/tseshared.[^/]+)/tseserver.static'
    m = re.match(format, name)
    if m is None:
        return name
    return '%s.debug/tseserver.static.debug' % m.group(1)

if __name__ == '__main__':
  f = None
  opts, args = getopt.getopt(sys.argv[1:], "ds", ["demangle", "split"])
  for o, a in opts:
    if o in ("-d", "--demangle"):
      demangle = True;
    if o in ("-s", "--split"):
      f = filter
  loop = SymbolizationLoop(f)
  loop.process_stdin()
