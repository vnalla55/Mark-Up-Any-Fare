...
DiamondValidator::validate(const SopIdVec& combination) const
{
  ...
  {
    if (obDetails->cxrCode[0] != obDetails->cxrCode[1])
    {
      for (const SopDetails* ibDetails : ibDetailsVec)
      {
        if (ibDetails->cxrCode[0] != ibDetails->cxrCode[1] &&
...
bool
DiamondValidator::isThrowAway(const SopIdVec& combination, ValidatorBitMask validBitMask) const
{
  return false;
}
