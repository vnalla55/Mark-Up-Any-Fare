...
sub genReportHTML
{
    ...
    my($testTitle, $testDate, $targetDir, $htmlFileName, 
       ...
       $startDate, $endDate, $instrHost, $testType, $acPort, $configsComparision) = @_;
    ...
    $self->genHTML($absoluteHtmlFile, $testTitle, $testDate,
                   ...
                   $startDate, $endDate, $instrHost, $testType, $acPort, $configsComparision);
...
sub genHTML
{
    ...
    $self->addConfigsInfo($htmlFH, $configsComparision);
...
{
    my $self = shift;
    my ($htmlFH, $configsInfo) = @_;

    my @tab = split('\n', $configsInfo);

    print $htmlFH "<h2>Config's diff</h2>";
