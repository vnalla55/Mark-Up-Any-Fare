...
sub initConnection
{
    ...
    my $dbh = DBI->connect("dbi:Oracle:host=$host;port=1521;service_name=INSTRI_ADHOC", $username, $password);
       #|| die "Cannot connect to db: '$DBI::errstr'";
    print "DBError -- Unable to connect to database.\n" if (not defined $dbh);
    print "Connected to database. \n" if($debug and defined $dbh);
    return $dbh;
}
...
sub buildSql
{
    ...
    my $tableName = $INSTR_TABLE_PREFIX;
...
sub getMinuteAvgStats
{
    ...
    $whereClause .= " and transactiondatetime > to_timestamp('" . $startdate . "', 'YYYY-MM-DD HH24:MI:SS')";
    $whereClause .= " and transactiondatetime < to_timestamp('" . $enddate . "', 'YYYY-MM-DD HH24:MI:SS')";
...
sub getAvgStatsByBusinessFunction
{
    ...
    #no need to iterate through each day because oracle tables are not by day
    #foreach my $day (@days)
    {
        print "processing for day : $day \n" if($debug);
        my $sql = $self->buildSql($selectList, $day, $whereClause, $groupBy);
...
sub getAvgFareCounters
{
    ...
    foreach my $day (@days)
    {
        print "processing for day : $day \n" if($debug);
        my $sql = "SELECT " . $selectList;
        $sql .= " FROM " .  $INSTR_TABLE_PREFIX . " B, " . $FSR_INSTR_TABLE_PREFIX . " F";
        $sql .= " WHERE " . $whereClause;
