...
sub handle_start
{
	...
	if($element eq 'ITN') {
		...
		if($args{'parentsonly'} and $seen_q5q{$attr{'Q5Q'}}) {
			++$skipping;
			return;
		}
		...
		if($args{'itin'} and $itn != $args{'itin'} or
		   $args{'nitins'} and $itn > $args{'nitins'}) {
			++$skipping;
			return;
		}
	}
	...
	if($element eq 'PRO' and $args{'estimates'}) {
		$attr{'Q1S'} = $args{'estimates'};
	}
