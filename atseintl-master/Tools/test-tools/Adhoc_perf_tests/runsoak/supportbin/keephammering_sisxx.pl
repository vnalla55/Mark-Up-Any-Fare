...
else
{
   ...
   print "                 10 => IS G Pool (Travelong)          \n";
   print "                 11 => MIP G Pool (Travelong)          \n";
   print "                 12 => IS H Pool (Travelocity Intl)          \n";
   print "                 13 => MIP H Pool (Travelocity Intl)          \n";
...
sub prepareRequests
{
    ...
    while( my $line = <INPUT> )
    {
        ...
        if( $inreq eq 1 )
        {
            if( $line =~ /^<BIL / )
            {
                $validreq = 1 if( $line =~ / C21="SISXX/ );
            }
            if( $line =~ /^<FGI / )
            {
                $validreq = 0 if( $line =~ / B70="JCB"/ );
            }
            $reqbuf .= $line;
        }
