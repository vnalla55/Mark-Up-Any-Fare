...
public:
  ...
  virtual ~FareClassAppSegInfo() { }

  static const int BK_CODE_SIZE = 8;                 // Bk Code vector max. size
  ...
  virtual bool operator==( const FareClassAppSegInfo & rhs ) const
  {
    bool eq = (    ( _directionality        == rhs._directionality        )
                && ( _bookingCodeTblItemNo  == rhs._bookingCodeTblItemNo  )
                && ( _overrideDateTblNo     == rhs._overrideDateTblNo     )
                && ( _minAge                == rhs._minAge                )
                && ( _maxAge                == rhs._maxAge                )
                && ( _tktCode               == rhs._tktCode               )
                && ( _tktCodeModifier       == rhs._tktCodeModifier       )
                && ( _tktDesignator         == rhs._tktDesignator         )
                && ( _tktDesignatorModifier == rhs._tktDesignatorModifier )
  ...
  static const FareClassAppSegInfo* emptyFareClassAppSeg() { return &_emptyFareClassAppSeg; };
