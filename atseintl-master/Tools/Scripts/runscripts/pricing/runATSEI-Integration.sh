...
umask 002
ulimit -c unlimited
cd /opt/atseintl/pricing
# perform log maintenance 
/bin/cat /dev/null > SAXParser.log
/bin/cat tserequest.log > Transactions.log
/usr/bin/tail -1 Transactions.log | /bin/mail -s "XML for core" joe.yglesias@sabre.com
/usr/bin/tail -1 Transactions.log | /bin/mail -s "XML for core" Mahannop.Jomtanawat@sabre.com
/usr/bin/tail -1 Transactions.log | /bin/mail -s "XML for core" binh.tran@sabre.com
/bin/cat /dev/null > tserequest.log
/bin/cat /dev/null > tseserver.log
/opt/FFC/bin/ServiceUtil -FFCConfig=/opt/atseintl/bin/ATSEIPricingLB.properties -FFCAppName=ATSEIPricingService -Op=deregister -ServiceTag=ATSEIPricingService -ORBInitRef NameService=corbaloc:iiop:directorycrt.sabre.com:27000/NameService
# run the tseserver.
exec /opt/atseintl/bin/tseserver -c /opt/atseintl/pricing/tseserverLB.cfg > /dev/null 2>&1
