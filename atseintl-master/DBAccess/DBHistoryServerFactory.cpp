...
namespace tse
{
// DBServerFactory::ServerType DBServerFactory::run_type = UNKNOWN_SERVER ;

log4cxx::LoggerPtr&
DBHistoryServerFactory::getLogger()
{
  static log4cxx::LoggerPtr logger(
      log4cxx::Logger::getLogger("atseintl.DBAccess.DBHistoryServerFactory"));
  return logger;
}

bool
DBHistoryServerFactory::isTimeForRefresh(void)
{
  bool rc = false;
...
DBHistoryServerFactory::DBHistoryServerFactory(tse::ConfigMan& config)
  : _config(config), _refreshTime(0), _refreshTimeout(0), _refreshTimeoutPercentVary(0)
{
  ...
  LOG4CXX_INFO(getLogger(),
               "Database Connection Refresh Timeout Variation is set to "
                   << _refreshTimeoutPercentVary << "%.");
}

DBHistoryServerFactory::~DBHistoryServerFactory() {}
