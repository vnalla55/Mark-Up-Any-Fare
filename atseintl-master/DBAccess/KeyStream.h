...
{
public:
  explicit KeyStream(int) : _pointer(0) { _buffer[0] = '\0'; }
  KeyStream(const std::string& name, int version) : _pointer(0)
  {
    char ch(0);
  ...
  int size() const { return _pointer; }
  ...
  friend inline bool elementToFlatKey(int element, KeyStream& stream)
  {
    char intStr[20] = "";
  ...
  friend inline bool elementToFlatKey(unsigned long element, KeyStream& stream)
  {
    char intStr[20] = "";
  ...
  friend inline bool elementToFlatKey(char element, KeyStream& stream)
  {
  ...
  friend inline bool elementToFlatKey(bool element, KeyStream& stream)
  {
    char ch(element ? '1' : '0');
    stream._buffer[stream._pointer++] = ch;
    stream._buffer[stream._pointer++] = '|';
  ...
  friend inline bool elementToFlatKey(long long element, KeyStream& stream)
  {
    char longLongStr[20] = "";
  ...
  friend inline bool elementToFlatKey(const DateTime& element, KeyStream& stream)
  {
      ...
      {
        std::cerr << "<Open Date[" << element << "]" << std::endl ;
        copy( "*open date*", stream ) ;
      }
      else if ( element.isInfinity() )
      {
        std::cerr << "<Infinity Date[" << element << "]" << std::endl ;
        copy( "*infinity date*", stream ) ;
      }
      else if ( element.isEmptyDate() )
      {
        std::cerr << "<Empty Date[" << element << "]" << std::endl ;
  ...
  friend inline bool elementToFlatKey(const char* element, KeyStream& stream)
  {
    if (LIKELY(element))
    {
      copy(element, stream);
  ...
  friend inline void endKey(KeyStream& stream)
  {
    ...
    if (LIKELY('|' == stream._buffer[stream._pointer - 1]))
    {
      stream._buffer[--stream._pointer] = '\0';
    }
    else
...
inline bool
elementToFlatKey(const struct nil_t&, KeyStream&)
{
  return false;
}
inline bool
elementToFlatKey(const std::string& element, KeyStream& stream)
{
  return elementToFlatKey(element.c_str(), stream);
}
inline bool
elementToFlatKey(const boost::container::string& element, KeyStream& stream)
{
  return elementToFlatKey(element.c_str(), stream);
}
template <size_t n, typename T>
bool
elementToFlatKey(const Code<n, T>& element, KeyStream& stream)
{
  return elementToFlatKey(element.c_str(), stream);
}
inline KeyStream& operator<<(KeyStream& stream, int key)
{
  elementToFlatKey(key, stream);
  endKey(stream);
  return stream;
}
inline KeyStream& operator<<(KeyStream& stream, const std::string& key)
{
...
template <size_t n, typename T>
KeyStream& operator<<(KeyStream& stream, const Code<n, T>& key)
{
  elementToFlatKey(key.c_str(), stream);
  endKey(stream);
  return stream;
