...
public:
  ...
  {
    return ((_orderNo == rhs._orderNo) && (_transitHours == rhs._transitHours) &&
            (_transitMinutes == rhs._transitMinutes) && (_sameDayInd == rhs._sameDayInd) &&
            (_nextDayInd == rhs._nextDayInd) && (_sameFlight == rhs._sameFlight) &&
            (_transitTaxonly == rhs._transitTaxonly) && (_transitDomDom == rhs._transitDomDom) &&
            (_transitDomIntl == rhs._transitDomIntl) && (_transitIntlDom == rhs._transitIntlDom) &&
            ...
            (_viaLocType == rhs._viaLocType) && (_viaLoc == rhs._viaLoc));
  }
...
private:
  ...
  int _flightArrivalHours = 0;
  int _flightArrivalMinutes = 0;
  int _flightDepartHours = 0;
  int _flightDepartMinutes = 0;
  LocType _viaLocType = LocType::UNKNOWN_LOC;
  LocCode _viaLoc;

  template <typename B, typename T>
  static B& convert(B& buffer, T ptr)
  {
    return buffer & ptr->_orderNo & ptr->_transitHours & ptr->_transitMinutes & ptr->_sameDayInd &
           ptr->_nextDayInd & ptr->_sameFlight & ptr->_transitTaxonly & ptr->_transitDomDom &
           ptr->_transitDomIntl & ptr->_transitIntlDom & ptr->_transitIntlIntl &
           ptr->_transitSurfDom & ptr->_transitSurfIntl & ptr->_transitOfflineCxr &
           ptr->_flightArrivalHours & ptr->_flightArrivalMinutes & ptr->_flightDepartHours &
           ptr->_flightDepartMinutes & ptr->_viaLocType & ptr->_viaLoc;
  }
...
template<> struct cdu_pod_traits<TaxRestrictionTransit>: std::true_type{};
