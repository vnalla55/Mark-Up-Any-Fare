...
{
public:
  TaxRestrictionPsg() : _showPsg(' '), _fareZeroOnly(' '), _minAge(0), _maxAge(0) {}
  ...
  {
    return ((_psgType == rhs._psgType) && (_showPsg == rhs._showPsg) &&
            (_fareZeroOnly == rhs._fareZeroOnly) && (_minAge == rhs._minAge) &&
            (_maxAge == rhs._maxAge));
  }
...
private:
  ...
  {
    return buffer & ptr->_psgType & ptr->_showPsg & ptr->_fareZeroOnly & ptr->_minAge &
           ptr->_maxAge;
  }
...
template<> struct cdu_pod_traits<TaxRestrictionPsg>: std::true_type{};
