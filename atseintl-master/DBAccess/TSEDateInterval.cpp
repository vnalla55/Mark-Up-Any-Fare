...
TSEDateInterval::TSEDateInterval(const TSEDateInterval& rhs) { rhs.cloneDateInterval(*this); }

const TSEDateInterval&
TSEDateInterval::
operator=(const TSEDateInterval& rhs)
{
  if (LIKELY(&rhs != this))
  {
...
TSEDateInterval::isEffective(const DateTime travelDT) const
{
  ...
  return travelDT >= _effDate && travelDT <= _expireDate && travelDT <= _discDate;
}

bool
TSEDateInterval::isEffective(std::pair<DateTime, DateTime> travelDateRange) const
{
  return travelDateRange.second >= _effDate && travelDateRange.first <= _expireDate &&
         travelDateRange.first <= _discDate;
}

bool
TSEDateInterval::isEffective(const DateTime ticketingDT, const DateTime travelDT) const
{
  ...
  return (ticketingDT.date() >= _createDate.date() || ticketingDT.date() >= _effDate.date()) &&
         ticketingDT.date() <= _expireDate.date() && ticketingDT.date() <= _discDate.date() &&
         travelDT >= _effDate && travelDT.date() <= _discDate.date();
}

bool
TSEDateInterval::defineIntersectionH(const TSEDateInterval& i1, const TSEDateInterval& i2)
{
  //      |--|=== i1 ===|--|
  //             |--|=== i2 ===|--|
  //  or
  ...
  if (i2.effDate() >= i1.effDate() && i2.effDate() <= i1.discDate())
  {
    _effDate = i2.effDate();
  }
  ...
  else if (i1.effDate() >= i2.effDate() && i1.effDate() <= i2.discDate())
  {
    _effDate = i1.effDate();
  }
...
bool
TSEDateInterval::defineIntersection(const TSEDateInterval& i1, const TSEDateInterval& i2)
{
  //      |--|=== i1 ===|--|
  //             |--|=== i2 ===|--|
  //  or
  ...
  if (i2.effDate() >= i1.effDate() && i2.effDate() <= i1.expireDate() &&
  ...
  else if (i1.effDate() >= i2.effDate() && i1.effDate() <= i2.expireDate() &&
...
bool
TSEDateInterval::defineUnion(const TSEDateInterval& i1, const TSEDateInterval& i2)
{
  //      |--|=== i1 ===|--|
  //             |--|=== i2 ===|--|
  // or
  ...
  else if (LIKELY(i1.effDate() >= i2.effDate() && i1.effDate() <= i2.expireDate() &&
           i1.effDate() <= i2.discDate()))
  {
    _effDate = i2.effDate();
  }
  ...
  if (UNLIKELY(i1.expireDate() < i2.expireDate()))
    _expireDate = i1.expireDate();
  else
    _expireDate = i2.expireDate();

  if (UNLIKELY(i1.discDate() < i2.discDate()))
    _discDate = i1.discDate();
  else
    _discDate = i2.discDate();
