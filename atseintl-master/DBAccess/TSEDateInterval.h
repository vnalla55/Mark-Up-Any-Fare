...
public:
  ...
  bool isEffective(const DateTime ticketingDT, const DateTime tarvelDT) const;

  bool isEffective(std::pair<DateTime, DateTime> range) const;

  bool defineIntersection(const TSEDateInterval& i1, const TSEDateInterval& i2);

  bool defineIntersectionH(const TSEDateInterval& i1, const TSEDateInterval& i2);

  bool defineUnion(const TSEDateInterval& i1, const TSEDateInterval& i2);
  ...
  bool operator!=(const TSEDateInterval& rhs) const { return (!(*this == rhs)); }

  friend inline std::ostream& dumpObject(std::ostream& os, const TSEDateInterval& obj)
  {
    return os << "[" << obj._createDate << "|" << obj._effDate << "|" << obj._expireDate << "|"
              << obj._discDate << "]";
  }
