...
sub test_Accessor_isAccessor
{
  my $variable = Gen::Accessor->new({
    name => 'isFoo',
    type => Gen::Type->new('bool')
  });

  assertStringEquals($variable->name, 'isFoo');
  assert(!$variable->isReference);
  assertStringEquals($variable->postfixName('Postfix'), 'fooPostfix');
  assertStringEquals($variable->getExpression, 'item->isFoo()');
  assertStringEquals($variable->setInstruction('true'), 'item->setFoo(true)');
  assertStringEquals($variable->setInstructionPointer('ptr'), 'item->setFoo(*ptr)');
}
