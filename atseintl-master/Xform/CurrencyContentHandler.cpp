//-------------------------------------------------------------------
//
//  File:        CurrencyContentHandler.cpp
...
{
static Logger
logger("atseintl.Xform.CurrencyContentHandler");

void
CurrencyContentHandler::startElement(const XMLCh* const uri,
                                     ...
                                     const Attributes& attrs)
{
  ...
  try
  {
    CurrencyModelMap& currencyModelMap = dynamic_cast<CurrencyModelMap&>(_dataModelMap);
    if (!currencyModelMap.classMapEntry(tagName, attrs))
    {
  ...
  {
    LOG4CXX_ERROR(logger, "DataModelMap is not a Currency model map");
  }
}

void
CurrencyContentHandler::endElement(const XMLCh* const uri,
                                   const XMLCh* const localname,
                                   const XMLCh* const qname)
{
  char* chTagName = XMLString::transcode(qname);
  ...
  {
    CurrencyModelMap& currencyModelMap = dynamic_cast<CurrencyModelMap&>(_dataModelMap);
    currencyModelMap.saveMapEntry(tagName);
  }
  catch (std::bad_cast&)
  {
    LOG4CXX_ERROR(logger, "DataModelMap is not a Currency model map");
...
const bool
CurrencyContentHandler::parse(const char*& content)
{
  bool retCode = true;
