//-------------------------------------------------------------------
//
//  File:        CbasBrandingServiceCallerTest.cpp
//  Created:     April 2013
//  Authors:
//
//  Description:
...
namespace tse
{
class CbasMockBrandingServiceCaller : public BrandingServiceCaller<CBAS>
{
public:
  CbasMockBrandingServiceCaller(PricingTrx& trx) : BrandingServiceCaller<CBAS>(trx) {}
};

class CbasBrandingServiceCallerTest : public CppUnit::TestFixture
{
  CPPUNIT_TEST_SUITE(CbasBrandingServiceCallerTest);
...
public:
  void setUp()
  {
    ...
    _memHandle.insert(_pricingServiceCallerMock = new CbasMockBrandingServiceCaller(*_pricingTrx));
    _memHandle.insert(_fareDisplayServiceCallerMock =
                          new CbasMockBrandingServiceCaller(*_fareDisplayTrx));
  ...
  void testPrintDiagnosticFdRequest()
  {
    ...
    _fareDisplayTrx->diagnostic().diagParamMap().insert(
        std::make_pair(Diagnostic::DISPLAY_DETAIL, "ABCCREQ"));
  ...
  void testPrintDiagnosticFdResponse()
  {
    ...
    createFdDiag(Diagnostic195);
    _fareDisplayTrx->diagnostic().diagParamMap().insert(
        std::make_pair(Diagnostic::DISPLAY_DETAIL, "ABCCRES"));
    _fareDisplayTrx->setTrxType(PricingTrx::FAREDISPLAY_TRX);
    _fareDisplayServiceCallerMock->printDiagnostic(
        request, response, StatusBrandingService::NO_BS_ERROR);
    std::string expected;
...
protected:
  ...
  CbasMockBrandingServiceCaller* _pricingServiceCallerMock;
  CbasMockBrandingServiceCaller* _fareDisplayServiceCallerMock;
  TestMemHandle _memHandle;
};

CPPUNIT_TEST_SUITE_REGISTRATION(CbasBrandingServiceCallerTest);
